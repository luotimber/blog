...
- Java 11 升Java 17 重要特性必读
  - 升级JDK17概述
    - [JDK 17升级的必要性？](#jdk-17升级的必要性)
    - [JDK 11 升级到JDK 17 性能提升多少？](#jdk-11-升级到jdk-17-性能提升多少)
    - [如何更好的理解从JDK 11 到 JDK 17 升级中带来的重要特性？](#如何更好的理解从jdk-11-到-jdk-17-升级中带来的重要特性)
  - 语言新特性
    - [JDK14 - Switch 表达式（JDK 12,13预览，14正式）](#jdk14---switch-表达式jdk-1213预览14正式)
    - [JDK15 - 文本块（JDK 13,14预览，15正式）](#jdk15---文本块jdk-1314预览15正式)
    - [JDK16 - instanceof 模式匹配（JDK 14,15预览，16正式）](#jdk16---instanceof-模式匹配jdk-1415预览16正式)
    - [JDK16 - Records类型（JDK 14,15预览，16正式）](#jdk16---records类型jdk-1415预览16正式)
    - [JDK17 - 密封的类和接口（JDK 15,16预览，17正式）](#jdk17---密封的类和接口jdk-1516预览17正式)
  - 新工具和库更新
    - [JDK13 - Socket API 重构](#jdk13---socket-api-重构)
    - [JDK14 - 改进 NullPointerExceptions 提示信息](#jdk14---改进-nullpointerexceptions-提示信息)
    - [JDK15 - 隐藏类 Hidden Classes](#jdk15---隐藏类-hidden-classes)
    - [JDK15 - DatagramSocket API重构](#jdk15---datagramsocket-api重构)
    - [JDK16 - 对基于值的类发出警告](#jdk16---对基于值的类发出警告)
    - JDK17 - 增强的伪随机数生成器
      - [**JDK 17之前如何生成随机数**？](#jdk-17之前如何生成随机数)
      - [**为什么需要增强**？](#为什么需要增强)
      - [**增强后是什么样的**？](#增强后是什么样的)
  - JVM优化
    - [JDK13 - 增强 ZGC 释放未使用内存](#jdk13---增强-zgc-释放未使用内存)
    - [JDK14 - G1 的 NUMA 可识别内存分配](#jdk14---g1-的-numa-可识别内存分配)
    - [JDK14 - 删除 CMS 垃圾回收器](#jdk14---删除-cms-垃圾回收器)
    - [JDK14 - 弃用 ParallelScavenge 和 SerialOld GC 的组合使用](#jdk14---弃用-parallelscavenge-和-serialold-gc-的组合使用)
    - [JDK15 - 禁用偏向锁定](#jdk15---禁用偏向锁定)
    - [JDK15 - 低暂停时间垃圾收集器](#jdk15---低暂停时间垃圾收集器)
    - [JDK16 - ZGC 并发线程处理](#jdk16---zgc-并发线程处理)
    - [JDK16 - 弹性元空间](#jdk16---弹性元空间)
  - [参考资料](#参考资料)

## 升级JDK17概述

> 这里帮你梳理为何JDK 17将会是一个极为重要的版本以及如何去理解它。@pdai
...
从规划调度引擎 OptaPlanner 项目（原文在[这里  (opens new window)](https://www.optaplanner.org/blog/2021/09/15/HowMuchFasterIsJava17.html)）对 JDK 17和 JDK 11 的性能基准测试进行了对比来看：
...
![image-20220720093955654](http://blogs.luckyluo.top:9000/blogimg/87a8e5c3-781e-4f6c-8ea3-e6eb6a647529.png)
...
![image-20220720094014879](http://blogs.luckyluo.top:9000/blogimg/9e81a871-a094-4ffe-9950-8160628b6f36.png)
