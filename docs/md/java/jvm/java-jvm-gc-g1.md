...
- GC - Java 垃圾回收器之G1详解
  ...
  - \2. G1的内存模型
    - 2.1 分区概念
      - [2.1.1 分区Region](#211-分区region)
      - [2.1.2 卡片Card](#212-卡片card)
      - [2.1.3 堆Heap](#213-堆heap)
    - 2.2 分代模型
      - [2.2.1 分代垃圾收集](#221-分代垃圾收集)
      - [2.2.2 本地分配缓冲 Local allocation buffer (Lab)](#222-本地分配缓冲-local-allocation-buffer-lab)
    - 2.3 分区模型
      - [2.3.1 巨形对象Humongous Region](#231-巨形对象humongous-region)
      - [2.3.2 已记忆集合Remember Set (RSet)](#232-已记忆集合remember-set-rset)
      - [2.3.3 Per Region Table (PRT)](#233-per-region-table-prt)
    - 2.4 收集集合 (CSet)
      - [2.4.1 年轻代收集集合 CSet of Young Collection](#241-年轻代收集集合-cset-of-young-collection)
      - [2.4.2 混合收集集合 CSet of Mixed Collection](#242-混合收集集合-cset-of-mixed-collection)
      - [2.4.3 并发标记算法（三色标记法）](#243-并发标记算法三色标记法)
      - [2.4.4 漏标问题](#244-漏标问题)
  - \3. G1的活动周期
    - [3.1 G1垃圾收集活动汇总](#31-g1垃圾收集活动汇总)
    - 3.2 RSet的维护
      - [3.2.1 栅栏Barrier](#321-栅栏barrier)
      - [3.2.2 起始快照算法Snapshot at the beginning (SATB)](#322-起始快照算法snapshot-at-the-beginning-satb)
      - [3.2.3 并发优化线程Concurrence Refinement Threads](#323-并发优化线程concurrence-refinement-threads)
    - 3.3 并发标记周期 Concurrent Marking Cycle
      - [3.3.1 并发标记线程 Concurrent Marking Threads](#331-并发标记线程-concurrent-marking-threads)
      - [3.3.2 初始标记 Initial Mark](#332-初始标记-initial-mark)
      - [3.3.3 根分区扫描 Root Region Scanning](#333-根分区扫描-root-region-scanning)
      - [3.3.4 并发标记 Concurrent Marking](#334-并发标记-concurrent-marking)
      - [3.3.5 存活数据计算 Live Data Accounting](#335-存活数据计算-live-data-accounting)
      - [3.3.6 重新标记 Remark](#336-重新标记-remark)
      - [3.3.7 清除 Cleanup](#337-清除-cleanup)
    - 3.4 年轻代收集/混合收集周期
      - [3.4.1 GC工作线程数](#341-gc工作线程数)
      - [3.4.2 年轻代收集 Young Collection](#342-年轻代收集-young-collection)
    - 3.5 并发标记周期后的年轻代收集 Young Collection Following Concurrent Marking Cycle
      - [3.5.1 混合收集周期 Mixed Collection Cycle](#351-混合收集周期-mixed-collection-cycle)
      - [3.5.2 转移失败的担保机制 Full GC](#352-转移失败的担保机制-full-gc)
  - [4. 总结](#4-总结)
  - [参考资料](#参考资料)
...
![image-20220720132645966](http://blogs.luckyluo.top:9000/blogimg/d8130379-a75e-41ec-9288-952057f852c1.png)
...
![image-20220720132656229](http://blogs.luckyluo.top:9000/blogimg/fb68400d-6f63-4616-9215-f68ddb4fa093.png)
...
![image-20220720132708763](http://blogs.luckyluo.top:9000/blogimg/05252608-64f8-478e-b2f9-50bdfcae5fa5.png)
...
![image-20220720132719948](http://blogs.luckyluo.top:9000/blogimg/6e240c81-30f9-4f96-9983-238f027007f6.png)
...
![image-20220720132732468](http://blogs.luckyluo.top:9000/blogimg/e17bf720-7a91-402d-9646-878cbe9dec62.png)
...
![java-jvm-gc-g1-5-1](http://blogs.luckyluo.top:9000/blogimg/a260cb59-48ca-40b9-a8f6-190d1907b085.gif)

#### 2.4.4 漏标问题

在remark过程中，黑色指向了白色，如果不对黑色重新扫描，则会漏标。会把白色D对象当作没有新引用指向从而回收掉。

![image-20220720132849834](http://blogs.luckyluo.top:9000/blogimg/930b8ac8-796d-488b-b184-24b06ce3acbd.png)
...
![image-20220720132904278](http://blogs.luckyluo.top:9000/blogimg/64570eac-9186-4a21-b231-b407d11c6b40.png)
...
![image-20220720132914004](http://blogs.luckyluo.top:9000/blogimg/cddb6674-e870-4764-98e3-de36773f36f9.png)
...
![image-20220720132929717](http://blogs.luckyluo.top:9000/blogimg/474694ec-a4c7-40b8-84df-b97fd5a34b93.png)
